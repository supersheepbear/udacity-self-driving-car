...
class CalculateCurvature:
    def __init__(self, warped_image, image_name):
...
window_centroids = find_window_centroids(self.warped, self.window_width, self.window_height, self.margin)
    def draw_windows
        ...
        if len(window_centroids) > 0:
            ...
            for level in range(0, len(window_centroids)):
                # self.window_mask is a function to draw window areas
                l_mask = self.window_mask(self.window_width, self.window_height, self.warped, window_centroids[level][0], level)
                r_mask = self.window_mask(self.window_width, self.window_height, self.warped, window_centroids[level][1], level)
            ...
            warpage = np.dstack((self.warped, self.warped, self.warped)) * 255  # making the original road pixels 3 color channels
